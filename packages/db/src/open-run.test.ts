...
const fakeTx = (
  ...
  return { tx: tx as never, creates, activities, taskUpdates, inbox, runFindFirstCalls, agentLocks: () => agentLocks };
};
...
for (const intent of reassignedRetryIntents) {
  ...
  test(`${intent.kind} replaces a retired native subagent pin with the current one`, async () => {
    ...
    const opened = await openRun(tx, "task-1", intent);
    assert.equal(opened.ok, true);
    assert.equal(creates[0]!.model, "gpt-5.6-sol:high");
...
test("a no-result repair retry uses its repair card ref despite a salvage publication", async () => {
  ...
  const task = taskRow({ repoId: repo.id, repo, chainId: null, chainIndex: null,
    maxSessionsPerTask: 2, targetBranch: shared, runs: [prior] });
  const { tx, creates } = fakeTx(task, {
    publishedRuns: [{ taskId: task.id, repoId: repo.id, pushedBranch: salvage }],
  });
  const opened = await openRun(tx, task.id, {
