...
test("one malformed recovery row cannot prevent another replay in the same tick", async () => {
  ...
  let healthy!: Awaited<ReturnType<typeof seedStopped>>;
  try {
    healthy = await seedStopped("canonical-direct", "recovery-regression-healthy-row");
    assert.equal((await baseDriftRecoveryTick(db, reader(snapshot(BASE_2)))).recovered, 1);
  ...
  } }), 1, "the same aggregate, Run and reason writes one durable error activity");
  assert.ok(replayErrors.length >= 2, "every isolated row failure remains visible in process logs");
  assert.equal(replayErrors[0]?.[0], "Recovery Regression replay row failed");
});
...
test("a CI recovery replay spends the next CI recovery birth", async () => {
  const seeded = await seedStopped("canonical-direct", "ci-replay-shared-budget", "check-failure-or-absence");
  assert.equal((await baseDriftRecoveryTick(db, ciReader())).recovered, 1);
  await failQueuedRecoveryRun(seeded.gateTask.id, true);
  assert.equal((await replayFailedRecoveryRegressions(db)).replayed, 1);
  await nextCiStop(seeded, HEAD_2);

  const next = await baseDriftRecoveryTick(db, ciReader(snapshot(BASE, {
    ...failedCiSnapshot(), headRefOid: HEAD_2, headCommitOid: HEAD_2,
  })));
  assert.equal(next.exhausted, 1);
  assert.equal(await db.run.count({ where: { taskId: seeded.gateTask.id, status: "QUEUED" } }), 0);
  assert.equal(await db.taskActivity.count({ where: {
    taskId: seeded.integratorTask!.id,
    metadata: { path: ["kind"], equals: "mergeTail.ciFailureRecovery" },
