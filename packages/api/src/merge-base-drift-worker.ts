...
const queueRecovery = async (
  ...
  const ciHistory = expected.recoveryKind === "ci-failure"
    ...
    : [];
  const ciStopIds = new Set(ciHistory.map((row) => asJsonObject(row.metadata)?.stopId)
    .filter((value): value is string => typeof value === "string"));
  const ciExternalReplays = expected.recoveryKind === "ci-failure" && ciStopIds.size > 0
    ? await tx.mergeRecoveryAttempt.findMany({ where: {
        integratorTaskId: expected.integratorTaskId,
        sourceStopId: { in: [...ciStopIds] },
      }, select: { externalReplayCount: true } })
    : [];
  const attempts = expected.recoveryKind === "ci-failure"
    ? ciHistory.length + ciExternalReplays.reduce((total, row) => total + row.externalReplayCount, 0)
...
export const replayFailedRecoveryRegressions = async (
  ...
  while (result.replayed + result.blocked < limit) {
    ...
    for (const row of rows) {
      ...
      const [failedRun, newestRun, repairOwner] = await Promise.all([
        tx.run.findUnique({
          where: { id: runId },
          select: {
      ...
      if (externalRefund && allowance.spent < allowance.ceiling) {
        ...
        if (attempt.outcome === "opened") {
          ...
          await tx.taskActivity.create({ data: {
            taskId: latest.regressionTaskId,
            actorType: "control-plane",
            body: `Recovery Regression Run ${failedRun.runNumber} failed externally; Run ${attempt.run.runNumber} queued under recovery ownership`,
            metadata: {
      ...
      return "blocked";
        ...
        try {
          await db.$transaction(async (tx) => {
            if (!await lockRecoveryChain(tx, row.integratorTaskId)) return;
            const existing = await tx.taskActivity.findFirst({ where: {
              taskId: row.regressionTaskId!,
              actorType: "control-plane",
              AND: [
                { metadata: { path: ["kind"], equals: "mergeTail.recoveryRegressionReplayError" } },
                { metadata: { path: ["aggregateId"], equals: row.id } },
                { metadata: { path: ["recoveryRunId"], equals: runId } },
