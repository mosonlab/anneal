...
import { requestMergeTailRepair, requestMergeTailRerun } from "../merge-tail-reentry.js";
...
export const registerTasksRoutes = (app: RouteApp, deps: RouteDeps): void => {
  ...
  app.get("/tasks/:taskId", async (context) => {
    ...
    const { toolEventsBySession, ttftEventsBySession } = await readRunMetricEvents(db, metricsSessionIds);
