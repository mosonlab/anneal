...
test("a merge executor that stays offline past the wait stops the tail by name", async () => {
  await withExecutorAllowlist(EXECUTOR_RUNNER_ID, async () => {
    ...
    const expired = new Date(OFFLINE_NOW.getTime() + (15 * 60_000));
    assert.deepEqual(
...
test("a later outage waits out its own ceiling rather than the task's whole history", async () => {
  await withExecutorAllowlist(EXECUTOR_RUNNER_ID, async () => {
    ...
    assert.equal(
      ((await offlineMarkers(seeded.readiness.id)).at(-1)!.metadata as Record<string, unknown>).episodeClosed,
      true,
    );
    ...
    assert.equal(
      (markers[2]!.metadata as Record<string, unknown>).episodeStartedAt,
...
test("a live skip tick closes outage A before a held Step resumes into outage B", async () => {
  await withExecutorAllowlist(EXECUTOR_RUNNER_ID, async () => {
    ...
    assert.equal(((await offlineMarkers(seeded.readiness.id)).at(-1)!.metadata as Record<string, unknown>).episodeClosed, true);
    await db.task.update({ where: { id: seeded.regression.id }, data: { status: TaskStatus.DONE } });
...
test("a tick whose readiness claim was replaced cannot close the offline episode", async () => {
  await withExecutorAllowlist(EXECUTOR_RUNNER_ID, async () => {
    ...
    assert.notEqual(((await offlineMarkers(seeded.readiness.id)).at(-1)!.metadata as Record<string, unknown>).episodeClosed, true);
  });
});
...
for (const width of [0, 2]) test(`executor return re-arms a ceiling stop with train width ${width} without discarding its recovery aggregate`, async () => {
  await withExecutorAllowlist(EXECUTOR_RUNNER_ID, async () => {
    ...
    assert.equal(deferred.status, TaskStatus.TODO);
    assert.equal(deferred.readinessClaimToken, null);
    assert.equal(deferred.readinessClaimExpiresAt, null);
    assert.equal((await db.mergeRecoveryAttempt.findUniqueOrThrow({ where: { id: aggregate.id } })).status,
    ...
    if (width > 0) {
      const nextTick = await readinessTick(db, reader([], snapshot({ baseSha: "d".repeat(40) })),
        new Date(expired.getTime() + 3_000), 5, releaseChainLease, runWithMergeLease,
        executorsAt(ONLINE_NOW), width);
      assert.equal(nextTick.claimed, 1, "train formation can reclaim a one-shot base-advanced defer");
    }
  });
});
...
test("a second identical outage ceiling reopens its previously closed stop notice", async () => {
  await withExecutorAllowlist(EXECUTOR_RUNNER_ID, async () => {
    ...
    await readinessTick(db, reader(), new Date(second.getTime() + (15 * 60_000)),
