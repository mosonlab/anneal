...
FETCH_TRANSIENT_RE='fetch failed|SSL_ERROR_SYSCALL|SSL_connect|unexpected EOF|early EOF|\bPost[[:space:]]+"[^"]+"[[:space:]]*:[[:space:]]*EOF\b|our servers are currently overloaded|\bmodel[[:space:]]+is[[:space:]]+(currently[[:space:]]+)?at capacity\b|connection (reset|closed|timed out|lost|aborted|refused)|RPC failed|Operation timed out|Failed to connect|Could not resolve host|Recv failure|socket hang ?up|HTTP( response)?[[:space:]]*(408|425|429|5[0-9][0-9])|status( code)?[[:space:]]*(408|425|429|5[0-9][0-9])|502 Bad Gateway|503 Service Unavailable|504 Gateway Timeout|ECONNABORTED|ECONNRESET|EHOSTUNREACH|ENETDOWN|ENETRESET|ENETUNREACH|EPIPE|ETIMEDOUT|EAI_AGAIN|ETIMEOUT'
FETCH_ACCESS_REFUSAL_RE='authentication failed|could not read Username|permission denied|forbidden|HTTP( response)?[[:space:]]*(401|403)|status( code)?[[:space:]]*(401|403)|bad credentials|authorization failed|\bunauthorized\b|invalid credentials|requested URL returned error:[[:space:]]*(401|403)\b|resource not accessible'
