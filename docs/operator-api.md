...
- A successful request creates the Project and, in the same transaction, one
  ...
  `code-reviewer-opus-medium`, and `senior-dev-opus-medium`) bound to that Environment, and
...
Task. A per-Run salvage ref wins only when its Run was created after the
publishing repair Run. If a queued Regression Run targets one of its own
per-Run refs but the recorded head matches neither the repaired head nor the
retry continuation, claim stops it as non-retryable `handoff-invalid` before
...
The former manual recovery of force-pushing the Run 1 ref to the repair head is
now rejected by this claim guard: the database still records the Run 1 ref's
old `headSha`. Retry the Regression Task directly instead.
...
three-requeue ceiling outside recovery. An active recovery aggregate instead
uses its two-requeue allowance and stays within that aggregate. At the ceiling
the old card stays OPEN with an explanation. A terminal read failure, unsafe
identity/ancestry, or failed
requeue enters the existing stop path with a recorded reason. Healthy OPEN
...
- The id lookup is unfiltered by Project. It returns the message's ordered
  ...
  Goal, Session, or gate Task identifies a Project;
  global messages return `project: null`.
