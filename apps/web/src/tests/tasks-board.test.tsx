...
test("Archive All confirms the project-wide Done scope even while one chain is visible", async () => {
  ...
  const settledAggregate = (chainId: string, chainName: string, taskId: string) => ({
    ...
    activation: { state: "settled" as const, predecessor: null, taskId, hold: null }, totalCost: null, firstRunStartedAt: null,
...
test("the model line is the run's snapshot, not the agent's current tier", () => {
  ...
  // claude-opus-5-5:medium showed as gpt-5.6-sol:high.
  const markup = card({
...
test("a card with no runs has no run row, and a card with a run reads exactly as before", () => {
  ...
  const withRun = card({
    latestRun: boardRun({ runNumber: 3 }),
  });
...
const chainRow = (options: {
  ...
  const aggregate: ChainAggregate = {
    ...
    totalCost: null, firstRunStartedAt: null, createdAt: "2026-08-15T00:00:00.000Z", updatedAt: "2026-08-16T00:00:00.000Z",
  };
  return task({
...
test("the Todo head starts the whole wave from one dialog and names the chain that refused", async () => {
  ...
  try {
    ...
    // hanging, but sized for the loaded gate worker rather than an idle host
    // (CONTRIBUTING.md, "Test timing on the gate worker"): the property is that Delta
    // is not waiting on Alpha's pending request, and half a second of wall clock
    // for a chain of fetch doubles and React renders is a measurement of the
    // host, not of the page. Cleared on the winning path, or the passing case
    // leaves a 15s timer holding the event loop open after the suite is done.
    let deadline: ReturnType<typeof setTimeout> | undefined;
    try {
      await Promise.race([
        deltaStarted,
        new Promise<never>((_resolve, reject) => {
          deadline = setTimeout(() => reject(new Error("Delta was held behind Alpha")), 15_000);
        }),
      ]);
    } finally {
