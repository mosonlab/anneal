...
test("the add-project runbook and public links cover A1 pull-request onboarding", () => {
  ...
  assert.match(addProjectRunbook, /`local` Environment[\s\S]*four[\s\S]*`senior-dev-luna-max`[\s\S]*`code-reviewer-sol-high`[\s\S]*`code-reviewer-opus-medium`[\s\S]*`senior-dev-opus-medium`/u);
